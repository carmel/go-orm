...
func ConnectAll() {
	...
	pgdsn := os.Getenv("orm_POSTGRES_DSN")
	mydsn := os.Getenv("orm_MYSQL_DSN")
	sqdsn := os.Getenv("orm_SQLITE_DSN")
...
// if we've used Place already somewhere in orm
type CPlace Place
...
func loadDefaultFixture(db *DB, t *testing.T) {
	tx := db.MustBegin()
	tx.MustExec(tx.Rebind("INSERT INTO person (first_name, last_name, email) VALUES (?, ?, ?)"), "Jason", "Moiron", "carmel@carmel.net")
...
func TestUsage(t *testing.T) {
	RunWithSchema(defaultSchema, t, func(db *DB, t *testing.T) {
		...
		if jason.Email != "carmel@carmel.net" {
			t.Errorf("Expecting Email of carmel@carmel.net, got %s", jason.Email)
		}
...
// tests that orm will not panic when the wrong driver is passed because
// of an automatic nil dereference in orm.Open(), which was fixed.
