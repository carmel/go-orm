...
// PrepareNamedContext returns an orm.NamedStmt
...
// PreparexContext returns an orm.Stmt instead of a sql.Stmt.
//
// The provided context is used for the preparation of the statement, not for
// the execution of the statement.
...
// QueryxContext queries the database and returns an *orm.Rows.
// Any placeholder parameters are replaced with supplied args.
...
// QueryRowxContext queries the database and returns an *orm.Row.
// Any placeholder parameters are replaced with supplied args.
...
// MustBeginTx starts a transaction, and panics on error.  Returns an *orm.Tx instead
// of an *sql.Tx.
//
...
// BeginTxx begins a transaction and returns an *orm.Tx instead of an
// *sql.Tx.
//
// The provided context is used until the transaction is committed or rolled
// back. If the context is canceled, the sql package will roll back the
...
// Connx returns an *orm.Conn instead of an *sql.Conn.
...
// transaction. Provided stmt can be either *sql.Stmt or *orm.Stmt.
